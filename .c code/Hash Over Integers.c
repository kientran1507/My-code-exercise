...
int insertKey(struct HashTable *hashTable, long key) {
...
int findKey(struct HashTable *hashTable, long key) {
    unsigned int index = hash(key);
...
int main() {
    ...
    // Delete the * after the first block
    char stop;
    scanf(" %c", &stop);  // Note the space before %c to consume any whitespace
